...
"use strict";
  ...
  $(window).hashchange(function(e){
    ...
    if (hash.length){
      var hash_parts = hash.split('&', 2); //2 - limit, may be changed if more than two arguments
      //console.log('hash_parts: '+hash_parts);
      for(var i in hash_parts) {
        //console.log('hash_parts['+i+']: '+hash_parts[i]);
        if(hash_parts[i].indexOf("#") === 0) { //begins with "#"
          var current_tab = hash_parts[i];
        }else if(hash_parts[i].indexOf("scrollto") === 0) { //begins with "scrollto"
          var scrollto = hash_parts[i].split('=', 2);
          var scrollto_value = '#'+scrollto[1].split(',');  
        }
      }
      if(current_tab.length && scrollto_value){
        //console.log('and current_tab: '+current_tab);
        //console.log('and scrollto_value: '+scrollto_value);
        $('#scrollto').addClass(scrollto_value);
        $('.tabs.tabs-top').easytabs('select', current_tab);
        $(window).scrollTo($(scrollto_value), 800, {offset: -70});
      }else if(current_tab.length){
        var scrolldiv_value = $('#scrollto').attr('class');
        //console.log('scrolldiv_value: '+scrolldiv_value);
        //console.log('or current_tab: '+current_tab);
        $('.tabs.tabs-top').easytabs('select', current_tab);
        if (scrolldiv_value.length){
          $(window).scrollTo($(scrolldiv_value), 800, {offset: -70});
          $('#scrollto').removeClass(scrolldiv_value);
          //console.log('scrolldiv_value removed: '+$('#scrollto').attr('class'));
        }else{
          //console.log('scrolldiv empty');
          $(window).scrollTo($('.navbar-header'), 800);
        }
      }
    }
  });
